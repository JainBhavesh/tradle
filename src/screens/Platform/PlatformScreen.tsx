  ...
  return (
    <BaseLayout style={styles.main}>
      <Header
        ...
        rightIcon={roll == 'Buyer' ? Images.BUYER_BOY : Images.SELLER_GIRL}
        logStyle={{ marginLeft: 10 }}
        renderHeader={'Select Platform'}
      />
          ...
          listData.map((item, i) => (
            <TouchableOpacity
              ...
              onPress={() => {
                if (i == 1) {
                  roll == 'Seller'
                    ? navigation.navigate('TAB_BAR_SCREEN')
              ...
              <View style={{ flexDirection: 'row', alignSelf: 'center' }}>
                ...
                {/* {item.desc.split(' ').map((word, index) => (
                  <Text
                    key={index}
                    style={index === 0 ? styles.desc : styles.desc2}>
                    {word}
