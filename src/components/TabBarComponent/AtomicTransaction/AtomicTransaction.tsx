...
const AtomicTransaction = ({ getValue, status }: any) => {
  const { isAggreed, setAgreeToggle }: any = useStore();

  const [isStatus, setIsStatus] = useState(false);
  const { sellerData, buyerData, styles } = useAtomic();
  ...
  const progressData = [
    {
      name: 'Produces and Signs All Paperwork',
      progress: 20,
      color: '#10EA8E',
    },
    { name: 'Settles Payment', progress: 50, color: '#EAE710' },
    {
      name: 'Transfers to the Legal Beneficiary',
      progress: 75,
      color: '#AE79ED',
    },
    {
      name: 'Creates a Delivery insurance Bond',
      progress: 30,
      color: '#F8A0C8',
    },
        ...
        : */}
          <>
            <Text
              ...
              style={styles.title1}>
              {'Tradal Trade Transaction'}
            </Text>
            <RowProgressBar data={progressData} />
            <Text preset="h2" style={styles.title2}>
              {'Transaction '}
              <Text preset="h2" color="green">
                Successful
              </Text>
            </Text>
