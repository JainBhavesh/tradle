...
const TradeScreen = ({ getValue, status }: any) => {
  ...
  const renderItem = ({ item }: { item: ItemData }) => {
    ...
    const backgroundColor =
      item.name === 'Tradal Trade' ? '#1e94d4' : '#ebf6fa';
    const newcolor = item.name === 'Tradal Trade' ? 'white' : 'black';
    return (
      <View>
        {item?.name == 'Revolut Escrow' && (
          <Text
            preset="h4"
            style={{
    ...
    <BaseLayout style={styles.main}>
      ...
      {selectedName == 'Escrow' && (
        <View>
          <Text
            preset="h3"
            style={{ textAlign: 'left', padding: 12, color: 'black' }}>
            {`$190,000`}
          </Text>
        </View>
      )}
